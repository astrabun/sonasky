 ...
 *
 * Ported from apps/sonasky-ref/src/helpers/handleResolver.ts, parameterized on
 * handleResolverUrl instead of importing a fixed const (this runs server-side,
 * not in the browser, so there's no CORS to work around either).
 */
export const handleResolver = {
  async resolve(
    ...
    try {
      const url = new URL("/xrpc/com.atproto.identity.resolveHandle", handleResolverUrl);
